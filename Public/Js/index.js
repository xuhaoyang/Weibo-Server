    ...
    **/
     ...
     */
     $('.turn').click(function () {
        ...
        //判断是否多重转发
        if(tid){
            author = $.trim(orgObj.find('.author a').html());
    ...
    $('.comment').toggle(function () {
        ...
        $.ajax({
            ...
            complete :function(){
                //加载完成后
                commentLoad.hide(); 
                commentList.show().find('textarea').val('').focus();        //伸缩隐藏回复框
            }
        });
...
function replace_weibo(content){
    ...
    return content = content.replace(/<a.*?>(.*?)<\/a>/ig, '$1');
